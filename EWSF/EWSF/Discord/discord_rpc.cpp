...
{
	NextConnect = std::chrono::system_clock::now() + std::chrono::duration<int64_t, std::milli>{
		ReconnectTimeMs.nextDelay()
	};
}
...
extern "C" DISCORD_EXPORT void Discord_UpdatePresence(const DiscordRichPresence* presence)
{
	{
		std::lock_guard<std::mutex> guard(PresenceMutex);
		QueuedPresence.length = JsonWriteRichPresenceObj(QueuedPresence.buffer, sizeof QueuedPresence.buffer, Nonce++,
		                                                 Pid, presence);
		UpdatePresence.exchange(true);
	}
	SignalIOActivity();
...
extern "C" DISCORD_EXPORT void Discord_RunCallbacks(void)
{
	...
	if (WasJustConnected.exchange(false))
	{
		...
		{
			DiscordUser du{
				connectedUser.userId, connectedUser.username, connectedUser.discriminator, connectedUser.avatar
			};
			Handlers.ready(&du);
		}
	}
