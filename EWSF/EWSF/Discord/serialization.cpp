...
struct WriteObject
{
	...
	template <typename T>
	WriteObject(JsonWriter& w, T& name) : writer(w)
	{
...
struct WriteArray
{
	...
	template <typename T>
	WriteArray(JsonWriter& w, T& name) : writer(w)
	{
		WriteKey(writer, name);
		writer.StartArray();
	}
...

	{
...

		{
			...
			if (presence != nullptr)
			{
				...
				if (presence->largeImageKey && presence->largeImageKey[0] || presence->largeImageText && presence->
					largeImageText[0] || presence->smallImageKey && presence->smallImageKey[0] || presence->
					smallImageText && presence->smallImageText[0])
				{
				...
				if (presence->partyId && presence->partyId[0] || presence->partySize || presence->partyMax || presence->
					partyPrivacy)
				{
